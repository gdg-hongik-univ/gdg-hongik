...
const SIZE_MAP: Record<LinkIconSize, number> = {
  sm: 20,
  md: 24,
} as const

export interface LinkIconProps extends ComponentPropsWithRef<'svg'> {
  size?: LinkIconSize
  color?: string
}

const LINK_VARIANTS: Record<LinkIconSize, { viewBox: string; path: (color: string) => ReactNode }> =
  {
    md: {
      viewBox: '0 0 24 24',
      path: (color) => (
